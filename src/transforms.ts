...
const transforms: transforms = {
  ...
  link: ({ data }) => {
    ...
    const anchorText = data.meta?.anchor || data.link
    ...
    if (MAIL_REGEX.test(linkText)) {
      return `<div class=\"link-tool\"><a class=\"link-tool__content link-tool__content--rendered\" target=\"_blank\" href=\"mailto:${linkText}\">${linkPreviewElement.join('')}<span class=\"link-tool__anchor\">${anchorText}</span></a></div>`
    }

    return `<div class=\"link-tool\"><a class=\"link-tool__content link-tool__content--rendered\" target=\"_blank\" rel=\"nofollow noindex noreferrer\" href=\"${data.link}\">${linkPreviewElement.join('')}<span class=\"link-tool__anchor\">${anchorText}</span></a></div>`
  },
